...
from .browser import login_and_run
...
def browser_automation(
    ...
    """Log into an arbitrary website and optionally perform extra actions.
    ...
    url:
        Login page URL.
    user_env, pass_env:
        Names of environment variables holding the username and password.
        **WARNING**: Never hard-code real credentials; use a secure store.
    actions:
        List of additional steps after login. Each step is a dictionary with
        an ``action`` key (``goto``, ``click``, ``type`` or ``wait``).
    """

    username = os.getenv(user_env)
    password = os.getenv(pass_env)
    # WARNING: credentials must come from environment variables or a credential manager
    if not username or not password:
        return "credentials not configured"

    return login_and_run(url, username, password, actions or [])
